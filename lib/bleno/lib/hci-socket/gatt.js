...
Gatt.prototype.setServicesOrig = function(services) {
  debug('SET SERVICES BEZ MITM');
  var deviceName = process.env.BLENO_DEVICE_NAME || os.hostname();

  var allServices = services;
/* GATTacker - we already have it from cloned device 
...
Gatt.prototype.setServices = function(services) {
  debug('BLENO GATT SETSERVICES MITM ');
  ...
  for (i = 0; i < allServices.length; i++) {
    ...
    var serviceHandle = service.startHandle;
    ...
    for (j = 0; j < service.characteristics.length; j++) {
      ...
      var characteristicHandle = characteristic.startHandle;
      var characteristicValueHandle = characteristic.valueHandle;

//      debug("CHAR HANDLE - Start: " + characteristicHandle + " Value: " + characteristicValueHandle);
 ...
 //     debug("DESCRIPTORS LEN " + characteristic.descriptors.length + " 2901: " + util.inspect(characteristic.descriptors, {showHidden: false, depth: null}));
//      debug("DESCRIPTORS LEN" + characteristic.descriptors.length + " 2901: " + util.inspect(characteristic.descriptors[0][0], {showHidden: false, depth: null}));

      for (var k = 0; k < characteristic.descriptors.length; k++) {
        var descriptor = characteristic.descriptors[k];
 //       debug("DESCRIPTOR: " + k + " UUID: " + descriptor.uuid + " H: " + descriptor.handle) ;
        var descriptorHandle = descriptor.handle;
      ...
      var callback = (function(valueHandle) {
        return function(result, data) {
          ...
          if (ATT_ECODE_SUCCESS === result) {
           ...
           callbackResponse = this.errorResponse(ATT_OP_ERROR, valueHandle, result);
          }
